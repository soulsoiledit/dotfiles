  ...
  config = lib.mkIf config.modules.hyprland.enable {
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        "$notify_vol" =
          ''notify-send -c osd -i audio-volume-medium --hint=int:value:$(pamixer --get-volume) $(pamixer --get-volume)'';
        "$notify_kbd" =
          ''notify-send -c osd "Current keyboard led brightness" $(asusctl -k | rg ".* (\S+)" -r "\$1")'';
        "$notify_led" =
          ''notify-send -c osd "Current keyboard led mode:" "$(rg '\s+current_mode: (\S+),$' -r '$1' /etc/asusd/aura.ron)"'';
